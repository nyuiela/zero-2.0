...
import { useBridgeControllerFiatPrices } from "../providers/query";
...
import { currencySymbolMap } from "../types/configs";
import { useIsHyperlanePlayground, useIsSuperbridge, useIsSuperbridgeTestnet } from "./use-is-hyperlane";
import { useDeployments } from "../hooks/use-deployment";
import { useHasSlippageSetting } from "../types/configs";
...
export const SettingsModal = () => {
  ...
  const slippage = useSettingsState.useSlippage();
  const setSlippage = useSettingsState.useSetSlippage();
  ...
  const hasSlippageSetting = useHasSlippageSetting();
          ...
          <div className="border rounded-[16px] divide-y divide-border">
            <div className="flex items-center justify-between p-4">
              ...
              >
                ...
                <SelectContent position={isWidget ? "item-aligned" : "popper"}>
                  {Object.keys(fiat.data?.data ?? {})
                    .filter((symbol) => currencySymbolMap[symbol])
                    .map((symbol) => (
                      <SelectItem key={symbol} value={symbol}>
                        {currencySymbolMap[symbol].flag} {symbol}
                      </SelectItem>
                    ))}
                </SelectContent>
              </Select>
            ...
            <div className="flex items-center justify-between p-4">
              ...
              >
                ...
                <SelectContent
                  ...
                  position={isWidget ? "item-aligned" : "popper"}
                >
                  {[
            ...
            {(isSuperbridge || isSuperbridgeTestnet) && <TokenLists />}
