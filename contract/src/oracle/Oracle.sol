...
contract OracleMaster is IOracleMaster, Ownable, Pausable, ReentrancyGuard {
    ...
    bytes4 public constant REGISTER_CAR_BRAND_SELECTOR = bytes4(keccak256("registerCarBrand(string,string,(uint256,uint256,uint256,uint256,uint256),address)"));
    bytes4 public constant UPDATE_ORACLE_SELECTOR = bytes4(keccak256("updateOracle(string,(uint256,uint256,uint256,uint256,uint256))"));
    ...
    event BrandPermissionManagerCreated(string indexed brandName, address indexed permissionManager, address indexed brandOwner);
    ...
    function registerCarBrand(
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, REGISTER_CAR_BRAND_SELECTOR),
        ...
        IBrandPermissionManager(permissionManagerAddress).initialize(
            brandName,
            address(this),
            brandOwner
        );
    ...
    function updateOracle(
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, UPDATE_ORACLE_SELECTOR),
    ...
    function deactivateOracle(string memory brandName) external override {
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, DEACTIVATE_ORACLE_SELECTOR),
    ...
    function reactivateOracle(string memory brandName) external {
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, REACTIVATE_ORACLE_SELECTOR),
    ...
    function incrementProductCount(string memory brandName) external {
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, INCREMENT_PRODUCT_COUNT_SELECTOR),
    ...
    function decrementProductCount(string memory brandName) external {
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, DECREMENT_PRODUCT_COUNT_SELECTOR),
    ...
    function batchUpdatePrices(string[] memory brandNames, uint256[] memory prices) external {
        ...
        require(
            msg.sender == owner() || 
            globalPermissionManager.hasPermission(msg.sender, BATCH_UPDATE_PRICES_SELECTOR),
    ...
    function getPermissionDetails(address account, bytes4 functionSelector) external view returns (IPermissionManager.Permission memory) {
