 ...
 */
contract ZeroNFT is IZeroNFT, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        Ownable(msg.sender) //   ERC721URIStorage()
        ERC721("ZeroNFT", "ZNFT")
    {
        require(
    ...
    function batchTransfer(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _checkAuthorized(from, msg.sender, tokenIds[i]);
    ...
    function hasNFTs(address _owner) public view returns (bool) {
    ...
    function _exists(uint256 _tokenId) internal view returns (bool) {
        address addresss = _ownerOf(_tokenId);
        if (addresss != address(0)) {
            return true;
        } else {
            return false;
        }
