...
interface IInitFunction {
    ...
    function sendRequest(uint64 subscriptionId, string[] calldata args, string memory _brand) external returns (bytes32 requestId);
    function getResponse(string memory _brand) external view returns (string memory);
} 