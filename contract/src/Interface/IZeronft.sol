...
interface IZeroNFT {
    ...
    // Transfer functions
    function transferZero(address from, address to, uint256 tokenId) external payable;
    ...
    function getTokenBrand(uint256 tokenId) external view returns (string memory);

    function getBrandTokens(string memory brandName) external view returns (uint256[] memory);
    ...
    // Brand checks
    function isBrandRegisteredAndActive(string memory brandName) external view returns (bool);

    function isBrandActive(string memory brandName) external view returns (bool);
    ...
    function updateProtocolAddresses(address _oracleMater, address reputation, address _carRegistry) external;

    function isOwner(uint256 _tokenid, address user) external returns (bool);

    function transferZeroFrom(address from, address to, uint256 tokenId) external;
}
