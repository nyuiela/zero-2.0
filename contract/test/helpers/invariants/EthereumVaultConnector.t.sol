...
import "../../../../src/Euler/vault/interfaces/IERC1271.sol";
...
contract EthereumVaultConnectorHandler is EthereumVaultConnectorScribble, Test {
    ...
    function setNonce(
        ...
        if (nonceLookup[addressPrefix][nonceNamespace] == type(uint256).max)
            return;
    ...
    function reorderCollaterals(
        ...
        if (
            index1 >= index2 ||
            int256(uint256(index2)) >=
            int256(uint256(accountCollaterals[account].numElements)) - 2
        ) {
            return;
        }
    ...
    function controlCollateral(
        ...
        result = super.controlCollateral(
            targetCollateral,
            onBehalfOfAccount,
            0,
            data
    ...
    function batch(BatchItem[] calldata items) public payable override {
        ...
        for (uint256 i = 0; i < items.length; i++) {
            ...
            if (bytes4(items[i].data) == VaultMock.checkAccountStatus.selector)
                return;
        }
    ...
    {
        return (
            new BatchItemResult[](0),
            new StatusCheckResult[](0),
            new StatusCheckResult[](0)
        );
    }
    ...
    {
        SetStorage storage accountControllersStorage = accountControllers[
            account
        ];
        ...
        if (numOfControllers == 0) return (true, "");
            ...
            return (
                false,
                abi.encodeWithSelector(EVC_ControllerViolation.selector)
            );
    ...
    function checkVaultStatusInternal(
        address
    ) internal pure override returns (bool isValid, bytes memory result) {
        isValid = true;
        result = "";
    }
