...
import "../../../../src/Euler/vault/Set.sol";
...
contract SetTest is Test {
    ...
    function callbackTestWithResult(
        address element
    ...
    function test_InsertRemove(address[] memory elements, uint64 seed) public {
        ...
        for (
            uint256 i = 0;
            i < elements.length && expectedNumElements < SET_MAX_ELEMENTS;
            ++i
        ) {
    ...
    function test_RevertIfTooManyElements_Insert(uint256 seed) public {
        ...
        for (uint256 i = 0; i < SET_MAX_ELEMENTS; ++i) {
            assertEq(
                setStorage.insert(
                    address(
                        uint160(
                            uint256(bytes32(keccak256(abi.encode(seed, i))))
                        )
                    )
                ),
                true
        ...
        setStorage.insert(
            address(
                uint160(uint256(bytes32(keccak256(abi.encode(seed, seed)))))
            )
        );
    }
    ...
    function test_SecondElement_Insert(
        address elementA,
        address elementB
    ) public {
    ...
    function test_ArrayElementDuplicate_Insert(
        address elementA,
        address elementB
    ) public {
        vm.assume(elementA != elementB);
    ...
    function test_Reorder(
        ...
        for (uint8 i = 0; i < numberOfElements; ++i) {
            setStorage.insert(
                address(
                    uint160(uint256(keccak256(abi.encode(i, firstElement))))
                )
            );
        }
    ...
    function test_SecondElement_Remove(
        address elementA,
        address elementB
    ) public {
        vm.assume(elementA != elementB);
    ...
    function test_ForEachAndClearWithResult(
        uint8 numberOfElements,
        address firstElement
    ) public {
        numberOfElements = uint8(bound(numberOfElements, 2, SET_MAX_ELEMENTS));
        ...
        bytes[] memory result = setStorage.forEachAndClearWithResult(
            callbackTestWithResult
        );
        ...
        for (uint8 i = 0; i < numberOfElements; ++i) {
            (address element, bool success, bytes memory data) = abi.decode(
                result[i],
                (address, bool, bytes)
            );
