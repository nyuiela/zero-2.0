...
contract AccountAndVaultStatusTest is Test {
    ...
    function test_RequireAccountAndVaultStatusCheck(
        ...
        vm.assume(
            numberOfAddresses > 0 && numberOfAddresses <= SET_MAX_ELEMENTS
        );
        ...
        for (uint256 i = 0; i < numberOfAddresses; i++) {
            accounts[i] = address(
                uint160(uint256(keccak256(abi.encode(i, seed))))
            );
        ...
        for (uint256 i = 0; i < numberOfAddresses; i++) {
            ...
            if (
                allStatusesValid ||
                (uint160(vault) % 3 == 0 && !alreadyExpectsRevert)
            ) {
            ...
            if (
                allStatusesValid ||
                (uint160(account) % 3 == 0 && !alreadyExpectsRevert)
            ) {
                assertTrue(
