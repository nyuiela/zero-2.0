...
contract SignerECDSA is EIP712, Test {
    ...
    function _buildDomainSeparator() internal view override returns (bytes32) {
        return
            keccak256(
                abi.encode(_TYPE_HASH, _hashedName, block.chainid, address(evc))
            );
    }
...
contract PermitTest is Test {
    address internal constant EIP_7587_PRECOMPILES =
        0x0000000000000000000000000000000000000100;
    address internal constant COMMON_PREDEPLOYS =
    ...
    function test_ECDSA_Permit(
        ...
        address msgSender = sender == address(0)
            ? address(uint160(uint256(keccak256(abi.encode(alice)))))
            : sender;
        data = abi.encode(keccak256(data));

        vm.assume(
            !evc.haveCommonOwner(alice, address(0)) &&
                !evc.haveCommonOwner(alice, EIP_7587_PRECOMPILES) &&
        ...
        evc.permit{value: msgSender.balance}(
            alice,
            sender,
            nonceNamespace,
            nonce,
    ...
    function test_RevertIfNestedPermit_Permit(
        ...
        bytes memory signature2 = signerECDSA.signPermit(
            alice,
            address(this),
            nonceNamespace,
            nonce + 1,
        ...
        bytes memory signature1 = signerECDSA.signPermit(
            alice,
            address(this),
            nonceNamespace,
            nonce,
            ...
            ? option % 2 == 0
                ...
                : address(
                    uint160(
                        bound(
                            uint160(alice),
                            uint160(EIP_7587_PRECOMPILES),
                            uint160(EIP_7587_PRECOMPILES) + 0xFF
                        )
                    )
                )
            : address(
                uint160(
                    bound(
                        uint160(alice),
                        uint160(COMMON_PREDEPLOYS),
                        uint160(COMMON_PREDEPLOYS) + 0xFF
                    )
                )
            );
        bytes19 addressPrefix = evc.getAddressPrefix(alice);
    ...
    function test_RevertIfSignerIsNotContractERC1271_Permit(
        ...
        vm.assume(
            !evc.haveCommonOwner(signer, address(0)) &&
                !evc.haveCommonOwner(signer, EIP_7587_PRECOMPILES) &&
                !evc.haveCommonOwner(signer, COMMON_PREDEPLOYS) &&
                signer != address(evc)
        );
        vm.assume(nonce > 0 && nonce < type(uint256).max);
    ...
    function test_RevertIfInPermitDisabledMode_Permit(
        uint256 privateKey,
        uint128 value,
        bytes memory data
    ) public {
    ...
    function test_RevertIfInPermit_SetPermitDisabledMode(
        ...
        bytes memory data = abi.encodeWithSelector(
            IEVC.setPermitDisabledMode.selector,
            addressPrefix,
            true
        );
        bytes memory signature = signerECDSA.signPermit(
