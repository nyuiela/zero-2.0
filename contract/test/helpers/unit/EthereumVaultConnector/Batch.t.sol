...
contract BatchTest is Test {
    ...
    function test_Batch(address alice, address bob, uint256 seed) external {
        ...
        emit CallWithContext(
            alice,
            evc.getAddressPrefix(alicesSubAccount),
            alicesSubAccount,
            otherVault,
            Vault.requireChecks.selector
        );
    ...
    function test_RevertIfInvalidBatchItem_Batch(
        address alice,
        uint256 value,
        bytes calldata data
    ) external {
    ...
    function test_RevertIfChecksReentrancy_AcquireChecksLock_BatchRevert_BatchSimulation(
        ...
        IEVC.StatusCheckResult[]
            memory expectedAccountsStatusCheckResult = new IEVC.StatusCheckResult[](
                1
            );
        IEVC.StatusCheckResult[]
            memory expectedVaultsStatusCheckResult = new IEVC.StatusCheckResult[](
                1
            );
        ...
        expectedVaultsStatusCheckResult[0].result = abi.encodeWithSignature(
            "Error(string)",
            "malicious vault"
        );
        ...
        assertEq(
            accountsStatusCheckResult[0].checkedAddress,
            expectedAccountsStatusCheckResult[0].checkedAddress
        );
        assertEq(
            accountsStatusCheckResult[0].isValid,
            expectedAccountsStatusCheckResult[0].isValid
        );
        assertEq(
            accountsStatusCheckResult[0].result,
            expectedAccountsStatusCheckResult[0].result
        );

        assertEq(vaultsStatusCheckResult.length, 1);
        assertEq(
            vaultsStatusCheckResult[0].checkedAddress,
            expectedVaultsStatusCheckResult[0].checkedAddress
        );
        assertEq(
            vaultsStatusCheckResult[0].isValid,
            expectedVaultsStatusCheckResult[0].isValid
        );
        assertEq(
            vaultsStatusCheckResult[0].result,
            expectedVaultsStatusCheckResult[0].result
        );
    }

    function test_RevertIfControlCollateralReentrancy_AcquireControlCollateralLock_Batch(
        address alice
    ) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.EVC_ControlCollateralReentrancy.selector
            )
        );
        evc.batch(new IEVC.BatchItem[](0));
    ...
    function test_BatchRevert_BatchSimulation(address alice) external {
        ...
        expectedAccountsStatusCheckResult[0].checkedAddress = alice;
        expectedAccountsStatusCheckResult[0].isValid = true;
        expectedAccountsStatusCheckResult[0].result = abi.encode(
        ...
        expectedVaultsStatusCheckResult[0].result = abi.encode(
...

        {
            ...
            try evc.batchRevert(items) {
                ...
                assertEq(
                    expectedAccountsStatusCheckResult.length,
                    accountsStatusCheckResult.length
                );
                assertEq(
                    expectedAccountsStatusCheckResult[0].checkedAddress,
                    accountsStatusCheckResult[0].checkedAddress
                ...
                assertEq(
                    expectedVaultsStatusCheckResult.length,
                    vaultsStatusCheckResult.length
                );
                assertEq(
                    expectedVaultsStatusCheckResult[0].checkedAddress,
                    vaultsStatusCheckResult[0].checkedAddress
                ...
                assertEq(
                    keccak256(expectedVaultsStatusCheckResult[0].result),
                    keccak256(vaultsStatusCheckResult[0].result)
                );
            }
        }
        ...
        expectedAccountsStatusCheckResult[0].result = abi.encodeWithSignature(
            "Error(string)",
