...
contract EthereumVaultConnectorHandler is EthereumVaultConnectorHarness {
    ...
    function handlerReorderCollaterals(
        address account,
        uint8 index1,
        uint8 index2
    ) external {
...
contract CollateralsManagementTest is Test {
    ...
    function test_CollateralsManagement(
        ...
        for (uint256 i = 1; i <= numberOfVaults; ++i) {
            ...
            if (collateralsPost.length > 1) {
                ...
                (collateralsPost[index1], collateralsPost[index2]) = (
                    collateralsPost[index2],
                    collateralsPost[index1]
                );
    ...
    function test_RevertIfControlCollateralReentrancy_CollateralsManagement(
        address alice
    ) public {
        vm.assume(alice != address(evc));
        address vault1 = address(new Vault(evc));
