...
import "../../../../src/Euler/vault/utils/EVCUtil.sol";
import "../../evc/EthereumVaultConnectorHarness.sol";
import "../../../../src/Euler/vault/interfaces/IVault.sol";
...
contract EVCClient is EVCUtil {
    ...
    function calledThroughEVCPayableUint(
        uint256 param
    ) external payable callThroughEVC returns (uint256) {
        require(msg.sender == address(evc), "Not EVC");
        return param;
    }

    function calledThroughEVCBytes(
        bytes calldata param
    ) external callThroughEVC returns (bytes memory) {
        require(msg.sender == address(evc), "Not EVC");
        return param;
    }
    ...
    function calledByEVCWithChecksInProgress()
        external
...
contract EVCUtilTest is Test {
...

        {
            ...
            bytes memory result = evc.call(
                ...
                abi.encodeWithSelector(
                    evcClient.calledThroughEVCUint.selector,
                    input
                )
            );
    ...
    function test_callThroughEVCPayableUint(
        uint256 input,
        uint64 value
    ) external {
...

        {
            ...
            uint256 result = evcClient.calledThroughEVCPayableUint{
                value: value
            }(input);
            assertEq(result, input);
...

        {
            ...
            bytes memory result = evc.call{value: value}(
                ...
                value,
                abi.encodeWithSelector(
                    evcClient.calledThroughEVCPayableUint.selector,
                    input
                )
            );
            assertEq(abi.decode(result, (uint256)), input);
    ...
    function test_callThroughEVCBytes(bytes calldata input) external {
...

        {
            ...
            bytes memory result = evc.call(
                ...
                0,
                abi.encodeWithSelector(
                    evcClient.calledThroughEVCBytes.selector,
                    input
                )
            );
    ...
    function test_callThroughEVCPayableBytes(
...

        {
            // call directly
            uint256 balance = address(evcClient).balance;
            bytes memory result = evcClient.calledThroughEVCPayableBytes{
                value: value
            }(input);
            assertEq(result, input);
            assertEq(address(evcClient).balance, balance + value);
...

        {
            ...
            bytes memory result = evc.call{value: value}(
                ...
                value,
                abi.encodeWithSelector(
                    evcClient.calledThroughEVCPayableBytes.selector,
                    input
                )
            );
            assertEq(abi.decode(result, (bytes)), input);
    ...
    function test_calledByEVCAccount_calledByEVCAccountOwner_msgSenderOnlyEVCAccount_msgSenderOnlyEVCAccountOwner(
        ...
        vm.assume(
            !evc.haveCommonOwner(caller, address(0)) &&
                !evc.haveCommonOwner(caller, address(evc))
        );
        vm.assume(id != 0);
        ...
        assertEq(
            evcClient.msgSenderOnlyEVCAccount(),
            address(uint160(uint160(caller) ^ id))
        );
    ...
    function test_msgSender(address caller) external {
        ...
        bytes memory result = evc.call(
            ...
            caller,
            0,
            abi.encodeWithSelector(evcClient.msgSender.selector)
        );
    ...
    function test_msgSenderForBorrow(address caller) external {
        ...
        evc.call(
            ...
            caller,
            0,
            abi.encodeWithSelector(evcClient.msgSenderForBorrow.selector)
        );
