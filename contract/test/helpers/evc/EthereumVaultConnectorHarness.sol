...
contract EthereumVaultConnectorHarness is EthereumVaultConnectorScribble {
        ...
        if (deferred) {
            ...
            executionContext = EC.wrap(
                EC.unwrap(executionContext) &
                    ~uint256(0xFF0000000000000000000000000000000000000000)
            );
        }
    }
    ...
    function verifyAccountStatusChecks() public view {
        for (uint256 i = 0; i < expectedAccountsChecked.length; ++i) {
            ...
            uint256 lastAccountStatusCheckTimestamp = accountControllers[
                expectedAccountsChecked[i]
            ].getMetadata();
            ...
            require(
                lastAccountStatusCheckTimestamp == block.timestamp,
                "verifyAccountStatusChecks/timestamp"
            );
        }
    }
}
